...
const INTERACTION_PATTERNS: readonly [RegExp, string][] = [
  [/(?:打开|访问|进入|前往|跳转到).{0,32}(?:https?:\/\/|www\.|[a-z0-9.-]+\.(?:com|cn|ai|net|org|io|co|app|dev|so|tech|cloud|xyz)|网站|网页|页面|链接|后台|app|平台|京东|淘宝|天猫|拼多多|小红书|微博|知乎|抖音|b站|bilibili|github|linkedin|amazon|gmail)/i, '中文打开页面'],
  ...
  [/\b(?:open|visit|go to)\s+(?:https?:\/\/|www\.|[a-z0-9.-]+\.(?:com|cn|ai|net|org|io|co|app|dev|so|tech|cloud|xyz)|(?:the\s+)?(?:website|webpage|page|link|dashboard|app|site)|(?:amazon|github|linkedin|gmail|reddit|youtube|instagram|tiktok|twitter|x\.com))\b/i, 'open page'],
  ...
  [/\bnavigate\s+to\s+(?:https?:\/\/|www\.|[a-z0-9.-]+\.(?:com|cn|ai|net|org|io|co|app|dev|so|tech|cloud|xyz)|(?:the\s+)?(?:website|webpage|page|link|dashboard|app|site)|(?:amazon|github|linkedin|gmail|reddit|youtube|instagram|tiktok|twitter|x\.com))\b/i, 'navigate to page'],
  ...
  [/\b(?:use|open|visit|go\s+to)\s+(?:https?:\/\/|www\.|[a-z0-9.-]+\.(?:com|cn|ai|net|org|io|co|app|dev|so|tech|cloud|xyz))\s+(?:to|and)\s+(?:calculate|compute|solve|evaluate)\b/i, 'use website calculator'],
  [/\b(?:use|open|visit|go\s+to)\s+(?:https?:\/\/|www\.|[a-z0-9.-]+\.(?:com|cn|ai|net|org|io|co|app|dev|so|tech|cloud|xyz))\s+(?:to|and)\s+(?:create|edit|update|save|add|upload|generate|make|publish|schedule|fill\s+out)\b/i, 'use website app'],
  [/\b(?:in|on)\s+(?:https?:\/\/|www\.|[a-z0-9.-]+\.(?:com|cn|ai|net|org|io|co|app|dev|so|tech|cloud|xyz))\s*,?\s+(?:create|edit|update|save|add|upload|generate|make|publish|schedule|fill\s+out)\b/i, 'domain-first app workflow'],
  [/\b(?:create|edit|update|save|add|upload|generate|make|publish|schedule|fill\s+out)\s+.{1,80}\s+(?:in|on|to)\s+(?:https?:\/\/|www\.|[a-z0-9.-]+\.(?:com|cn|ai|net|org|io|co|app|dev|so|tech|cloud|xyz))\b/i, 'action-first domain app workflow'],
  ...
  [/(?:在|用|通过|打开|访问|进入|前往).{0,40}(?:https?:\/\/|www\.|[a-z0-9.-]+\.(?:com|cn|ai|net|org|io|co|app|dev|so|tech|cloud|xyz)).{0,48}(?:计算|算一下|求值)(?!率|策略|分析|报告|模板)/i, '中文网站计算操作'],
  [/(?:在|用|通过|打开|访问|进入|前往).{0,40}(?:https?:\/\/|www\.|[a-z0-9.-]+\.(?:com|cn|ai|net|org|io|co|app|dev|so|tech|cloud|xyz)).{0,64}(?:创建|新建|编辑|更新|保存|添加|上传|生成|制作|发布|安排|预约|填写).{0,48}(?:草稿|海报|自动化|邮件|campaign|页面|landing page|表单|文档|项目|任务|帖子|图片|设计|邀请|日程|备注|工单|联系人|记录)/i, '中文域名应用操作'],
