...
const INTERACTION_PATTERNS: readonly [RegExp, string][] = [
  ...
  [/\b(?:draft|compose|create|edit|update|schedule|save|add|reply\s+to|upload|download|export|share|move|rename|attach|import)\s+.{1,80}\s+(?:in|on|to)\s+(?:gmail|slack|notion|github|google docs|google forms|google sheets|google slides|google calendar|google drive|dropbox|onedrive|icloud drive|linkedin|hubspot|salesforce|stripe|calendly|trello|asana|jira|shopify|zendesk|intercom|linear|monday(?:\.com)?)\b/i, 'draft in app'],
  [/\b(?:draft|compose|create|edit|update|schedule|save|add|reply\s+to|upload|download|export|share|move|rename|attach|import)\s+.{0,80}\b(?:github\s+(?:issue|pull request|pr)|jira\s+(?:bug|issue|ticket)|linear\s+(?:bug|issue|task)|asana\s+tasks?|trello\s+(?:card|board)|zendesk\s+(?:ticket|reply)|intercom\s+(?:note|message)|hubspot\s+contact|salesforce\s+(?:customer|note|record)|stripe\s+(?:receipt|payment)|google\s+calendar\s+(?:meeting|event)|google\s+drive\s+(?:file|folder)|dropbox\s+(?:file|folder|link)|onedrive\s+(?:file|folder|link))\b/i, 'object-first SaaS workflow'],
  [/\b(?:find|search|download|upload|export|share|move|rename|attach|import)\s+.{1,80}\s+(?:in|on|to)\s+(?:google drive|dropbox|onedrive|icloud drive|hubspot|salesforce|stripe(?:\s+dashboard)?|calendly|trello|asana|jira|shopify|zendesk|intercom|linear|monday(?:\.com)?)\b/i, 'find or download in SaaS app'],
