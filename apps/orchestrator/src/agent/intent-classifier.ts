...
const INTERACTION_PATTERNS: readonly [RegExp, string][] = [
  ...
  [/(?:在|用|通过).{0,24}(?:gmail|slack|linkedin|notion|飞书|github|google docs|google forms|google sheets|google slides|google calendar|hubspot|salesforce|stripe(?:\s+dashboard)?|calendly|trello|asana|jira).{0,64}(?:写|撰写|起草|创建|新建|编辑|更新|查找|搜索|下载|安排|预约|添加|保存).{0,48}(?:草稿|邮件|消息|页面|文档|会议纪要|计划|报名表|表单|帖子|表格|演示文稿|幻灯片|会议|日程|邀请|联系人|客户|备注|付款|收据|链接|看板|任务|bug|issue|pull request|pr)/i, '中文应用草稿操作'],
  ...
  [/\b(?:draft|compose|create|edit|update|schedule|save|add)\s+.{1,80}\s+(?:in|on)\s+(?:gmail|slack|notion|github|google docs|google forms|google sheets|google slides|google calendar|linkedin|hubspot|salesforce|stripe|calendly|trello|asana|jira)\b/i, 'draft in app'],
  [/\b(?:find|search|download)\s+.{1,80}\s+(?:in|on)\s+(?:hubspot|salesforce|stripe(?:\s+dashboard)?|calendly|trello|asana|jira)\b/i, 'find or download in SaaS app'],
  [/发布(?:到|在)(?:小红书|微博|知乎|抖音|b站|bilibili|twitter|x\.com|linkedin|reddit)/i, '发布到平台'],
];

const NON_EXECUTION_PLATFORM_NAMES =
  '(?:gmail|linkedin|slack|notion|github|amazon|youtube|instagram|twitter|x\\.com|reddit|shopify|飞书|google forms|google docs|google sheets|google slides|google calendar|google flights|hubspot|salesforce|stripe|calendly|trello|asana|jira)';
