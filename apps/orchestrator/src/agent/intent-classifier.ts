...
const INTERACTION_PATTERNS: readonly [RegExp, string][] = [
  ...
  [/\b(?:use|open|visit|go\s+to)\s+(?:https?:\/\/|www\.|[a-z0-9.-]+\.(?:com|cn|ai|net|org|io|co|app|dev))\s+(?:to|and)\s+(?:calculate|compute|solve|evaluate)\b/i, 'use website calculator'],
  [/\b(?:use|open|visit|go\s+to)\s+(?:https?:\/\/|www\.|[a-z0-9.-]+\.(?:com|cn|ai|net|org|io|co|app|dev))\s+(?:to|and)\s+(?:create|edit|update|save|add|upload|generate|make|publish|schedule|fill\s+out)\b/i, 'use website app'],
  ...
  [/(?:在|用|通过|打开|访问|进入|前往).{0,40}(?:https?:\/\/|www\.|[a-z0-9.-]+\.(?:com|cn|ai|net|org|io|co|app|dev)).{0,48}(?:计算|算一下|求值)(?!率|策略|分析|报告|模板)/i, '中文网站计算操作'],
  [/(?:在|用|通过|打开|访问|进入|前往).{0,40}(?:https?:\/\/|www\.|[a-z0-9.-]+\.(?:com|cn|ai|net|org|io|co|app|dev)).{0,64}(?:创建|新建|编辑|更新|保存|添加|上传|生成|制作|发布|安排|预约|填写).{0,48}(?:草稿|海报|自动化|邮件|campaign|页面|landing page|表单|文档|项目|任务|帖子|图片|设计|邀请|日程|备注|工单|联系人)/i, '中文域名应用操作'],
