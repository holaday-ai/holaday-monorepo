...
## 结构化输出（最终结果）
**凡是"结果是多条可以对齐比较的数据"，最终输出必须用 markdown 表格，不要用段落或 bullet 列表**。适用场景：
- 比价 / 对比（平台 A vs 平台 B vs ...）
- 岗位列表 / 薪资对比
- 酒店 / 航班 / 商品列表
- 多来源信息汇总（排行、评分、榜单）

表格要求：
1. 第一列通常是"平台 / 来源 / 序号"这类锚，第二列起是具体属性（价格、链接、备注等）
2. 每列表头写中文
3. 表格**下方**补一行一句话结论，格式如 "**结论：淘宝便宜 ¥50，推荐首选。**"
4. 如果某列是 URL，写成 \`[平台名](URL)\` 不要裸 URL

比价示例（必须复刻这个结构）：

| 平台 | 产品 | 价格 | 链接 |
|------|------|------|------|
| 京东自营 | AirPods Pro 2 | ¥1,899 | [京东](https://item.jd.com/...) |
| 淘宝天猫 | AirPods Pro 2 | ¥1,849 | [天猫](https://detail.tmall.com/...) |

**结论：淘宝便宜 ¥50，建议首选。**

绝对不要写成："京东自营 1899 元，淘宝天猫 1849 元，淘宝便宜 50 块"——这种纯文字描述属于明确违规。`;
