...
### 交易/预约类任务的最小信息检查
对预订机票/酒店/餐厅、预约服务、报名活动、投递职位、发送邮件/消息、加购/结账等真实执行任务，启动浏览器前先判断是否缺少关键参数。
...
- 需要登录账号
- 涉及支付 / 下单（先展示金额和明细）
- 涉及预订 / 预约 / 报名 / 投递 / 加购 / 结账 / 取消订阅 / 退订等交易、半交易或破坏性流程：可以搜索、筛选、填写非敏感信息，但**不要点击最终确认 / 提交预约 / 提交报名 / 提交申请 / 确认预订 / Place order / Delete / Unsubscribe**。先展示对象、时间、价格、费用、收件人/申请对象、关键条款或将要改变的账户状态，让用户确认。
- 涉及发送消息 / 邮件（先展示内容预览）
