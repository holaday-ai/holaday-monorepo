...
export async function runSupercarTask(opts: RunSupercarOptions): Promise<SupercarOutcome> {
  ...
  try {
    while (iteration < maxIterations && Date.now() < deadline && !cancelled) {
      ...
      // Append the full assistant content — NEVER just the text. The
      // Anthropic API needs thinking-block signatures preserved round-
      // trip for tool use + adaptive thinking to work.
