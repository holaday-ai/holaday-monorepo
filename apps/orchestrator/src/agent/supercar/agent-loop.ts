...
export async function runSupercarTask(opts: RunSupercarOptions): Promise<SupercarOutcome> {
  ...
  try {
    while (iteration < maxIterations && Date.now() < deadline && !cancelled) {
      ...
      // Append the assistant turn. We keep thinking / text / tool_use
      // (thinking-block signatures MUST round-trip for adaptive thinking
      // + client tool use to work) but strip `server_tool_use` and
      // `server_tool_result` blocks.
      //
      // Why strip them: Sonnet 4.6 under computer-use-2025-11-24
      // implicitly enables server-side `code_execution` alongside
      // `web_search`. Each code_execution invocation creates a sandbox
      // container; if those blocks are carried into the next messages.
      // create call we MUST also pass `container: { id }` (otherwise
      // the API returns 400 "container_id is required when there are
      // pending tool uses generated by code execution with tools").
      //
      // Simplest fix: whitelist thinking / text / tool_use on the
      // replayed assistant turn. Claude can re-invoke server tools
      // next turn against a fresh container — for our use case
      // (browse-and-report) the per-turn sandbox is fine, we don't
      // need cross-turn state. The specific *_tool_result names the
      // SDK emits (web_search_tool_result, code_execution_tool_result,
      // bash_code_execution_tool_result, etc.) evolve per beta, so a
      // whitelist is more robust than a blacklist.
      const keepKinds: ReadonlySet<string> = new Set([
        'thinking',
        'redacted_thinking',
        'text',
        'tool_use',
      ]);
      const keepBlocks = response.content.filter((b) => keepKinds.has(b.type));
      messages.push({ role: 'assistant', content: keepBlocks });
