...
3. 两者的取景和身体比例必须大致相容，例如近景对应近景、半身对应半身、全身对应全身。只比较主角照片和参考帧的可见身体范围与大致尺度，不要要求两者保持同一姿态。只要任一参考帧清楚出现手臂、双手、腹部或腿部，而这些区域在主角照片中没有完整可见，就必须返回 fail 并标记 framing_mismatch；不要假设模型可以可靠补全缺失身体。姿态、手势不同不能单独作为 framing_mismatch。手臂和双手已经完整可见时，即使接近画面边缘也不视为缺失。明显的头肩照替换半身或全身动作、身体可见范围或比例严重不匹配均不通过。
