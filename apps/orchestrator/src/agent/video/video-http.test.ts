...
import { downloadToBuffer, fetchWithTimeout, VideoHttpError } from './video-http.js';

describe('fetchWithTimeout', () => {
  it('returns the response on success', async () => {
    const fetchImpl = (async () => new Response('ok', { status: 200 })) as unknown as typeof fetch;
    const res = await fetchWithTimeout('https://x', { method: 'GET' }, { timeoutMs: 1000, fetchImpl });
...
describe('downloadToBuffer', () => {
  ...
  it('throws on non-2xx', async () => {
    ...
    await expect(downloadToBuffer('https://x', { fetchImpl })).rejects.toBeInstanceOf(VideoHttpError);
  });
