...
describe('decideAuthTokenAction', () => {
  ...
  it('fresh token, no knownBad → set', () => {
    expect(decideAuthTokenAction('eyJnew', null, null)).toEqual({
      kind: 'set',
      token: 'eyJnew',
    });
    expect(decideAuthTokenAction('eyJnew', 'eyJold', null)).toEqual({
      kind: 'set',
      token: 'eyJnew',
    });
  });
  ...
  it('fresh token, different knownBad → set (only the SAME knownBad is refused)', () => {
    expect(decideAuthTokenAction('eyJnew', null, 'eyJrejected')).toEqual({
      kind: 'set',
      token: 'eyJnew',
    });
  });
  ...
  it('incoming matches knownBad + stored is null → refuse (cycle-breaker)', () => {
    ...
    expect(decideAuthTokenAction('eyJrejected', null, 'eyJrejected')).toEqual({
      kind: 'refuse',
  ...
  it('incoming matches knownBad + stored differs → still refuse', () => {
    ...
    expect(decideAuthTokenAction('eyJrejected', 'eyJold', 'eyJrejected')).toEqual({
      kind: 'refuse',
      reason: 'known_bad_token',
    });
  });
  ...
  it('stored matches knownBad but incoming differs → set (token rotated)', () => {
    ...
    expect(decideAuthTokenAction('eyJnew', 'eyJrejected', 'eyJrejected')).toEqual({
      kind: 'set',
      token: 'eyJnew',
    });
  });
