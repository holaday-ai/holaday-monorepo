...
describe('normalizeCdpNavigateUrl', () => {
  it('accepts http and https urls after trimming', () => {
    ...
    expect(normalizeCdpNavigateUrl('localhost:3000/app')).toBe('http://localhost:3000/app');
...
describe('executeCdpAction', () => {
  ...
  it('preserves plus and minus keys in shortcut descriptors', async () => {
    ...
    await expect(executeCdpAction(11, { kind: 'key', key: 'ctrl+minus' })).resolves.toEqual({
      ok: true,
      message: 'key ctrl+minus',
    });
...
describe('getActiveTabId', () => {
  ...
  it('focuses the selected tab window for fallback browser control', async () => {
