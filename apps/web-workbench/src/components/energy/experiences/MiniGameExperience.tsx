...
interface MiniGameExperienceProps {
...
export function MiniGameExperience({
  phase,
  onPhaseChange,
  onComplete,
}: MiniGameExperienceProps): JSX.Element {
  const [hits, setHits] = React.useState(0);

  React.useEffect(() => {
    if (phase === 'active') setHits(0);
  }, [phase]);

  const catchEnergy = (): void => {
    const nextHits = Math.min(TOTAL_ROUNDS, hits + 1);
    setHits(nextHits);
    if (nextHits === TOTAL_ROUNDS) {
      onComplete();
  ...
  if (phase === 'result') {
    return (
      <div className="energy-game-result" aria-live="polite">
        ...
        <h3>能量收集完成</h3>
        <p>你刚刚完成了一次短暂的注意力切换。带着这点轻盈，再回到今天。</p>
      </div>
    );
  }
