...
const STOCK_DECISION_CONFLICTS: readonly RegExp[] = [
  ...
  /(?:帮我|替我|给我|请|直接|立即|马上|代我|为我)(?:直接|立即|马上|执行|决定|去)?(?:买入|买进|购入|购买|卖出|卖掉|抛掉|抛售|出掉|买|卖|下单|清仓|建仓|加仓|减仓)/,
...
const SENSITIVE_IDENTITY_TERM =
  `(?:性别|性取向|同性恋|异性恋|双性恋|无性恋|年龄|\\d{1,3}岁(?:以下|以上|以内|以外)?|民族|${SPECIFIC_ETHNICITY_TERM}|种族|${SPECIFIC_RACIAL_IDENTITY_TERM}|宗教|${SPECIFIC_RELIGIOUS_IDENTITY_TERM}|残障|残疾|婚姻|婚育|孕育|怀孕|户籍|籍贯|出生地|国籍|外籍|${SPECIFIC_NATIONALITY_TERM}|政治面貌|中共党员|预备党员|党员|共青团员|团员|群众|民主党派|无党派人士|健康状况|疾病|${SPECIFIC_HEALTH_STATUS_TERM}|男性|女性|男士|女士|男(?=候选人|人才|简历|员工|人员|生|$)|女(?=候选人|人才|简历|员工|人员|生|$)|孕妇|已婚|未婚)`;
...
const HIRING_DIRECT_DECISION_CONFLICTS: readonly RegExp[] = [
  ...
  /(?:哪个|哪位|哪些)(?:候选人|人才).{0,6}(?:应该|应当|适合|最适合)?(?:录用|淘汰|拒绝)/,
...
function isReviewedContractPromise(
  ...
  const referencedTermIndex = suffix.search(/(?:的(?:合同|条款|协议|约定)|(?:条款|约定))/);
  return (
