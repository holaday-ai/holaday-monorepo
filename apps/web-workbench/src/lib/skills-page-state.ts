...
const HIRING_HUKOU_JOB_TITLE =
  /(?:招聘|招|找|筛选|选择|挑选)(?:(?!户籍|户口|候选人|人才|简历).){0,4}户口(?:登记|管理|户籍|业务|窗口|服务|咨询)(?:(?!男性|女性|男士|女士|已婚|未婚|怀孕|孕妇).){0,4}(?:员|师|警|官|工|顾问|主管|经理|主任|助理|客服|前台)(?:候选人|人才|简历)/;
...
const SKILL_BOUNDARY_CONFLICTS: Readonly<Record<string, readonly RegExp[]>> = {
  ...
  'project-delivery-management': [
    ...
    /(?:然后|之后|随后|接着|继而|同时|并且|而且|顺便|另外|转头|最后|再).{0,4}(?:照做|照此执行|按此执行|就这么做|执行上述(?:操作|修改|调整|删除)|按上述(?:方案|方式|操作)执行)/,
  ...
  'performance-review-design': [
    ...
    /(?:根据|按照|按|依据).{0,6}(?:绩效|考核).{0,10}(?:调整|调节|上调|下调|提高|增加|减少)(?:员工|人员)?(?:奖金|工资|薪资|薪酬)/,
...
const SKILL_BOUNDARY_SAFE_MENTIONS: Readonly<Record<string, readonly RegExp[]>> = {
  ...
  'performance-review-design': [
    /^(?:分析|说明|解释|检查|审查|评估).{0,30}(?:为什么)?(?:不能|无法|不应|不要).{0,14}(?:根据|按照)?.{0,8}(?:绩效|考核).{0,12}(?:决定|执行|加薪|提薪|涨薪|降薪|辞退|晋升|上调|下调)/,
    /^(?:分析|检查|审查|评估).{0,24}(?:根据|按照)?.{0,8}(?:绩效|考核).{0,12}(?:(?:直接)?(?:决定|执行|加薪|提薪|涨薪|降薪|辞退|晋升)|(?:上调|下调|提高|增加|减少)(?:员工|人员)?.{0,4}(?:工资|薪资|薪酬|奖金)).{0,16}(?:风险|合规|边界|问题|后果)/,
  ],
};
...
const HIRING_DIRECT_DECISION_CONFLICTS: readonly RegExp[] = [
  ...
  /(?:这个|该|这位|该位)?(?:候选人|人才).{0,6}(?:适合|值得|应该|应当|可以|能否|能|可).{0,4}(?:录用|入职)(?:吗|么)?/,
