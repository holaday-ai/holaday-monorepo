...
const SKILL_BOUNDARY_CONFLICTS: Readonly<Record<string, readonly RegExp[]>> = {
  ...
  'project-delivery-management': [
    /(?:未经授权|自动|替我|代我).{0,10}(?:指派|分配|修改|删除|移除).{0,14}(?:项目|任务|负责人)/,
    /(?:未经授权|自动|替我|代我).{0,10}(?:把|将)?.{0,14}(?:项目|任务|负责人).{0,8}(?:指派|分配|修改|删除|移除|改成|改为)/,
    /(?:替|代替).{0,8}(?:团队|负责人|项目组).{0,8}(?:承诺|确认|决定).{0,12}(?:最终)?(?:排期|截止时间|交付时间)/,
    /(?:替我|代我|为我).{0,8}(?:向客户|对客户)?.{0,6}承诺.{0,12}(?:项目)?.{0,6}(?:最终)?(?:排期|截止时间|交付时间)/,
    /^(?:把|将).{0,14}(?:项目|任务|负责人).{0,8}自动(?:指派|分配|修改|删除|移除|改成|改为)/,
    /(?:项目|任务|负责人).{0,8}自动(?:指派|分配|修改|删除|移除|改成|改为)/,
  ],
