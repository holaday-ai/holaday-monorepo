...
const HIRING_HUKOU_JOB_TITLE =
  /(?:户籍|户口)(?!的|户籍|户口|候选人|人才|简历|要求|限制|优先)(?:(?!男性|女性|男士|女士|已婚|未婚|怀孕|孕妇).){0,6}(?:员|师|警|官|工|顾问|主管|经理|主任|助理|客服|前台)(?:候选人|人才|简历)/;
...
const CONTENT_EXECUTION_ANALYSIS =
  /^(?:(?:请|帮我|替我|给我|麻烦)(?:先|再)?){0,2}(?:分析|复盘|研究|总结|解释|评估|检查|审查).{0,40}(?:发布|发表|上线|上传|推送|发到|发至|发出去|投流|投放|买量).{0,24}(?:效果|数据|表现|结果|原因|问题|风险|影响|趋势|复盘|合规|边界)/;
...
const CONTENT_PROMISE_ANALYSIS =
  /^(?:分析|说明|解释|检查|审查|评估|复盘|研究).{0,32}(?:保证|承诺|确保|保底).{0,24}(?:风险|问题|后果|原因|边界|可行性|合规)$/;
...
const SKILL_BOUNDARY_CONFLICTS: Readonly<Record<string, readonly RegExp[]>> = {
  ...
  'project-delivery-management': [
    ...
    /(?:(?:帮我|替我|代我|为我)?.{0,4}(?:向客户|对客户).{0,4}(?:承诺|保证|答应|确认)|(?:承诺|保证|答应)客户).{0,16}(?:项目)?.{0,8}(?:在|于)?(?:今天|明天|后天|本周|下周|本月|下月|下个月|\d{1,2}月\d{1,2}[日号]?|\d{1,3}(?:天|周|个月)(?:内)?)(?:前|内)?交付/,
  ...
  'performance-review-design': [
    ...
    /(?:根据|按照|按|依据).{0,6}(?:绩效|考核).{0,10}(?:给|让).{0,4}(?:员工|人员)?.{0,4}(?:加薪|涨薪|降薪|辞退|解雇|晋升|降职|奖金|薪酬)/,
    /(?:根据|按照|按|依据).{0,6}(?:绩效|考核).{0,10}(?:辞退|解雇|晋升|降职|加薪|涨薪|降薪|发放奖金).{0,4}(?:员工|人员)?/,
    /(?:绩效|考核).{0,12}(?:后|然后|再).{0,8}(?:把|将)?.{0,4}(?:员工|人员).{0,4}(?:开了|辞退|解雇)/,
    /(?:根据|按照|按|依据).{0,6}(?:绩效|考核).{0,10}(?:扣(?:除)?(?:员工|人员)?(?:奖金|工资|薪资|薪酬)|降低(?:员工|人员)?(?:工资|薪资|薪酬))/,
    /(?:根据|按照|按|依据).{0,6}(?:绩效|考核).{0,10}(?:调整|调节)(?:员工|人员)?(?:奖金|工资|薪资|薪酬)/,
...
const SKILL_BOUNDARY_SAFE_MENTIONS: Readonly<Record<string, readonly RegExp[]>> = {
  ...
  'product-plan-drafting': [
    ...
    /^(?:分析|检查|审查|评估).{0,12}(?:合同|条款|协议).{0,24}(?:自动|替我|代我|直接)?(?:批准|通过|确认|认定|拍板).{0,12}(?:产品)?(?:需求|方案).{0,12}(?:条款|约定|风险|合规|边界|问题)/,
  ...
  'performance-review-design': [
    /^(?:分析|说明|解释|检查|审查|评估).{0,30}(?:为什么)?(?:不能|无法|不应|不要).{0,14}(?:根据|按照)?.{0,8}(?:绩效|考核).{0,12}(?:决定|执行|加薪|涨薪|降薪|辞退|晋升)/,
    /^(?:分析|检查|审查|评估).{0,24}(?:根据|按照)?.{0,8}(?:绩效|考核).{0,12}(?:直接)?(?:决定|执行|加薪|涨薪|降薪|辞退|晋升).{0,16}(?:风险|合规|边界|问题|后果)/,
  ],
};
...
const HIRING_DIRECT_DECISION_CONFLICTS: readonly RegExp[] = [
  ...
  /^(?:建议|推荐).{0,6}(?:录用|淘汰|拒绝)(?:哪位|哪个|哪些|谁).{0,8}(?:候选人|人才)?/,
  /^(?:建议|推荐).{0,8}(?:哪位|哪个|哪些|谁).{0,8}(?:候选人|人才)?.{0,6}(?:入职|录用|淘汰|拒绝)/,
  /(?:哪个|哪位|哪些)(?:候选人|人才).{0,6}(?:应该|应当|适合|最适合)?(?:入职|录用|淘汰|拒绝)/,
  /(?:这个|该|这位|该位)?(?:候选人|人才).{0,6}(?:适合|值得|应该|应当|可以|能否).{0,4}(?:录用|入职)(?:吗|么)?/,
...
function splitBoundaryClauses(normalizedIntent: string): string[] {
  return normalizedIntent
    .split(
      /(?:然后|之后|随后|接着|继而|同时|并且|但是|但|再|(?:并|以及)(?=(?:把|将|为我|帮我|替我|直接|自动|未经|没有|未|出具|提供|给出|代替|替代|相关性|相关关系|保证|承诺|确保|保底|批准|通过|认定|确认|决定|指派|分配|修改|涨薪|加薪|晋升|降级|辞退|解雇|淘汰|拒绝|录用|只招|只要|仅限|只选|只筛选|仅筛选|筛选|筛掉|过滤|排除|不招|不录用|优先考虑|发布|发表|上线|上传|推送|投流|投放|买量|交易|买入|卖出))|(?:完成|检查|分析|规则|看完|审完|写好|改好|准备好|评估)后)|[，,；;。.!！？?]+/,
    )
    .filter(Boolean);
}
