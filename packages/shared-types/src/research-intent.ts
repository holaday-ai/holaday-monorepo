...
export function isResearchOrRetrievalIntent(intent?: string | null): boolean {
  ...
  const transformsSuppliedContent =
    /^(?:(?:请|麻烦)?\s*)?(?:(?:把|将)(?:以下|下面|下列|这(?:段|句|篇|份)|上述|所附)|(?:翻译|改写|润色|总结|摘要)(?:以下|下面|下列|这(?:段|句|篇|份)|上述|所附|\s*[:：]))/i.test(
      text,
    );
  if (hasFreshnessCue && hasDynamicFactCue && !transformsSuppliedContent) {
    return true;
  }
