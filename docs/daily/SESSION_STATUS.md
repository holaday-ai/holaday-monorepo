...
## 🔴 PROD LIVE REF = `claude/musing-keller-ae1d05` — orch `ec4e023e`（**④ explorer v1 骨架 dark ship · `EXPLORER_ENABLED` 未设=OFF=绝不跑 · restart 682** · supercar llm_calls 记账 · ① crystallize 离线 CLI · migration 0037 · ACTION_CAPTURE+B4_SCREENSHOT_ANCHOR ON 灰度 · B3_FIXTURE OFF）/ SPA `8da47b4b`（bundle `index-DiYh_GAx.js`，未变）
