...
## 🔴 PROD LIVE REF = `claude/musing-keller-ae1d05` — orch `3476cac`（**④ explorer 捕获环(→crystallize) + browse-intent 收敛 + totalUsers 排除系统行 dark ship · restart 694 · `EXPLORER_ENABLED`+`EXPLORER_VETO_FIXTURE_ENABLED` 都 OFF（进程实证、fixture 404）=绝不跑 · `--browse` LOCK1 gate 住、browser 栈仅 --browse 分支 lazy-import、捕获环只在 explorer CLI（tasks.ts 0 改）=用户任务不变** · batch-1 figma 真跑 $0.4275 落地 · cost-source A fail-closed 熔断 + (a)非有限 fail-closed +(c)EXPLORER_MAX_ITERATIONS env + requireBrowseEnv abort · 护栏夹具 8/8 · veto 多信号 OR · ① crystallize(确定性零烧钱) · 记账系统身份 users.id=16 playbook-explorer · migration 0037 · ACTION_CAPTURE+B4 ON · B3_FIXTURE OFF）/ SPA `8da47b4b`（未变）
