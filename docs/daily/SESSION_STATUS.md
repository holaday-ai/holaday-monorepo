...
## 🔴 PROD LIVE REF = `claude/musing-keller-ae1d05` — orch `6f96bed`（**④ browse lane wired 进 explore-sites `--browse`（cost-source A 内存累加 fail-closed 熔断 + requireBrowseEnv 缺-id abort）dark ship · restart 690 · `EXPLORER_ENABLED`+`EXPLORER_VETO_FIXTURE_ENABLED` 都 OFF（进程实证、fixture 路由 404）=绝不跑 · `--browse` 在码但 LOCK1 gate 住不自动跑、browser 栈仅 --browse 分支 lazy-import=用户任务不变** · 护栏夹具 8/8 PASS · veto 多信号 OR · supercar 记账 · ① crystallize · migration 0037 · ACTION_CAPTURE+B4 ON · B3_FIXTURE OFF）/ SPA `8da47b4b`（bundle `index-DiYh_GAx.js`，未变）
