...
## 🔴 PROD LIVE REF = `claude/musing-keller-ae1d05` — SPA + orch `27b05395` / bundle `index-CjJR2v0s.js`（**Partner Ledger 表/代码暗态上线 + 媒体预览不卡 loading + AkShare 非交易时段 smoke 修复 LIVE（2026-07-13 JST）**：Partner Ledger migration 0040/0041 已 apply 且 schema verify 通过，`PARTNER_LEDGER_ENABLED` 未开启；SPA 双端 Aliyun/Vultr smoke 过，登录态首页/视频/图片/股票/Partner 关闭页验收通过；图片/视频历史预览全链路 8s 超时保护已上线，过期/卡住文件不再无限显示“图片加载中”；AkShare health/rankings smoke 过，非交易时段分时不可用只警告不阻塞部署；此前状态机防复活/防误恢复 guard、技能 planner 闭环、未成功任务清理口径、批量任务需复核计数拆分、技能 dispatch、用量加载态、任务详情 evidence fallback、批量子任务状态保真、股票最近真实交易日分时继续生效。）
