...
## 🔴 PROD LIVE REF = `claude/musing-keller-ae1d05` — SPA `b528bb6f` / orch `1dd219f7` / bundle `index-D2lOEeiu.js`（**Partner Ledger 暗态 + 媒体预览不卡 loading + AkShare 非交易时段 smoke + 首页图全局 preload/媒体下载预览控制台噪音修复 + 非 A 股股价查询不再被 A 股专线劫持 LIVE（二次巡检至 2026-07-15 JST）**：Partner Ledger migration 0040/0041 已 apply 且 schema verify 通过，`PARTNER_LEDGER_ENABLED` 未开启；SPA 双端 Aliyun/Vultr smoke 过，登录态首页/视频/图片/股票/Partner 关闭页验收通过；图片/视频历史预览全链路 8s 超时保护已上线，过期/卡住文件不再无限显示“图片加载中”，预览 fetch 与点击下载失败均由 UI 消化、不再打 console error；AkShare health/rankings/intraday smoke 过，2026-07-15 生产 8848 health/rankings/intraday 复验通过；双域名 HTML shell 都指向 `index-D2lOEeiu.js`，跟随主域 302 后两域 bundle sha256 一致；登录态 12 路由/响应式组合 Playwright 巡检无 console warn/error、无登录回退、无横向溢出（2026-07-15 复跑仍为 0 issue）；真实任务 smoke：文本翻译 5s completed，Tesla/TSLA 股价查询部署后 30s completed、返回来源链接且未进入 A 股引导兜底；2026-07-15 生产 canary：纯文本 9s completed（`tsk_g2b9BBewpxHfw6KbroGcg`），A 股 603528 44s completed（`tsk_vEA3m9gChgdEeWCR3q3aY`，结果标 AkShare/Sina 来源且不构成投资建议）；web typecheck/test/build、orch typecheck/test/build/db:verify、shared-types typecheck、cn-payment typecheck/test 均通过，2026-07-15 生产 `db:verify` 复验通过；生产 supercar smoke 脚本已移除明文测试凭证、改为强制读取 `HOLADAY_EMAIL`/`HOLADAY_PASSWORD`；此前状态机防复活/防误恢复 guard、技能 planner 闭环、未成功任务清理口径、批量任务需复核计数拆分、技能 dispatch、用量加载态、任务详情 evidence fallback、批量子任务状态保真、股票最近真实交易日分时继续生效。）
