...
## 🔴 PROD LIVE REF = `codex/trust-loop-round1` — orch `a60358f`（**状态机清理 LIVE（2026-07-03）**：离开 `awaiting_user` 的所有 repository 持久化路径会清空 `awaitingQuestion`/`awaitingKind`，避免 completed/cancelled/paused 等非等待状态残留“仍在等用户”的脏字段；前序 `92000d7` terminal side-effect race guard 已在同分支 LIVE；SPA 未变。）
