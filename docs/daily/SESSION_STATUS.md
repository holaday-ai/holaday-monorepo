...
账户关闭 rollout 的只读预检已补齐：`dormant`、`canary-ready`、`canary-running` 三种模式统一检查双公网 health、5 张 closure 表与活动队列聚合、Orchestrator/worker 数量、uid、RSS、监听端口及必要配置是否存在；输出只允许固定检查名、布尔值、计数和长度，不打印密钥、allowlist 条目、模板 ID、用户 ID 或队列内容。新门禁已纳入 `test:ops`，完整运维套件 59 项 Node 测试与全部 shell 发布/回滚/运行时门禁通过。2026-08-27 JST 生产只读复验中，`dormant` 为 6/6 ready（四个开关均 false、worker 0、活动队列 0）；`canary-ready` 为 7/11 blocked，缺口严格限定为双人遗留清理确认、专用 HMAC、单一 synthetic allowlist 确认和账户关闭短信配置。该检查没有修改生产数据、进程、配置、环境变量、部署或 Git 状态，账户关闭仍未向用户开放。
