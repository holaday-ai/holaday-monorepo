...
股市任务侧栏意图预加载已通过 PR #124 收口上线：鼠标进入或键盘聚焦“股市任务”时只预取约 89.9 kB（gzip 29.1 kB）的路由模块，不请求股票数据、不写浏览器存储；真正点击后仍只启动一组自选列表、简报状态与可信仪表盘请求，并继续复用 PR #122 的同组 promise。测试先行记录缺少路由预加载与侧栏意图回调时的失败后完成修复，定向 29/29、web-workbench 完整套件 195 个文件 / 1476 项、lint、typecheck、生产构建及 `git diff --check` 均通过。application 从 `24d0bf9cef37b71b4a142aaa290b8d823af4e3ed` 安全快进部署到 `789702bacb4aaae636ede66ab071ec0152db662f`；编号 migration 核对为 13 statements / 183 already applied、`db:verify`、快速 P0 发布门禁 4/4、31 项运维发布测试及 Aliyun/Vultr 原子 SPA smoke 均通过。Vultr 远端 HEAD 与合并提交一致，Orchestrator online、pid 1330694、uid 998、restart count 0；生产机本地及双公网 healthz 均为 `status=ok`，双站 SPA 均引用 `index-DVH-tMDO.js`。登录态生产四次独立标签页从侧栏键盘意图预取后进入 `/stocks`，页面壳为 3,084–3,115ms、08/21 可信内容为 3,094–3,340ms；对比发布前同路径可信内容中位数约 3,129ms，发布后约 3,112ms，仅改善约 0.5%，属于测量噪音范围，不能宣称用户可感知提速。实测约 3.07–3.09 秒主要耗在点击后的股票数据链路，路由模块预取只缩短很小的前端资源阶段；先前观察到的约 14.6 秒选股偏好首开本轮未能复现（首次约 1.8 秒，缓存后约 0.74–0.86 秒），因此未改后端、数据库或偏好缓存。风险证据仍稳定显示“A股检查 5/5 只”，页面控制台无 warning/error，验证后已恢复默认关注股票视图并关闭新增测试标签。验证只读浏览生产页面，没有生成股票任务、修改用户数据、schema、生产配置、密钥、支付、AkShare、DivineAPI、Translator 或 OpenAI Key；下一性能主项应针对点击后约 3.1 秒的数据请求做分段耗时与服务端瓶颈定位。
