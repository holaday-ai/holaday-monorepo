...
## 🔴 PROD LIVE REF = `claude/musing-keller-ae1d05` — orch `9e0f23fc`（**④ browse-试用 v1 dark ship（agent-loop onBeforeAction live-veto + clean-context 模式 + runBrowseTask + exploration_runs writer + 红队夹具）· restart 683 · `EXPLORER_ENABLED`+`EXPLORER_VETO_FIXTURE_ENABLED` 缺失=OFF=绝不跑、夹具零 prod 面 · clean-context 默认 off=用户任务字节级不变** · supercar llm_calls 记账 · ① crystallize · migration 0037 · ACTION_CAPTURE+B4 ON 灰度 · B3_FIXTURE OFF）/ SPA `8da47b4b`（bundle `index-DiYh_GAx.js`，未变）
