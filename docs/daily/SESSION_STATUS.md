...
## 🔴 PROD LIVE REF = `claude/musing-keller-ae1d05@180accaa`（2026-09-04 JST）
...
千问下一调用点盘点后，产品确认不保留自动“模型二次润色层”：主模型结果应直接进入已有终态验证与前端确定性排版，避免额外时延、费用和事实被改写的风险。旧实现的 URL/数字/标记检查只是有限的单向新增与标记类型保护，不能称为完整事实保留门禁。本轮代码收口将实际润色实现移除，所有任务 lane 通过永久透传兼容边界返回主模型原文；旧 `OPENAI_RESPONSE_LAYER_ENABLED` 与密钥即使误配置也不能恢复第二次模型调用，新任务不再写润色审计列，历史字段仅保留兼容旧记录。独立的 OpenAI verifier fallback 与视频等专用调用点不属于此次范围。
