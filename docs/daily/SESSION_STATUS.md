...
## 🔴 PROD LIVE REF = `codex/trust-loop-round1` — orch `d627115`（**状态机清理 LIVE（2026-07-03）**：重启/重连恢复会重播 durable `server.supercar.awaiting_user` 等待态，避免登录/补充信息暂停刷新后不可见；前序 `aafaae2` 已让 result patch helper 清 completed/awaiting_user 的旧 `pauseReason`/error/awaiting 字段；前序 `c3fe3fa` 已让 video confirm/cancel/reprompt raw SQL 统一清旧 `pauseReason`/error；前序 `3c1429f` 已让 maintenance/repository 状态出口清旧 `pauseReason`；前序 `b1d37c6` 已清 awaiting/error 残留与 controller stale error；前序 `92000d7` terminal side-effect race guard 已在同分支 LIVE；SPA 未变。）
