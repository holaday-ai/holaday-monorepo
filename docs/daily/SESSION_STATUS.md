...
## 🔴 PROD LIVE REF = `claude/musing-keller-ae1d05@052d6117`（2026-09-04 JST）

千问 provider-neutral Messages 适配器与首个低风险调用点已通过 PR #203–#207 暗发布上线。适配器统一文本、图片、工具、缓存提示、请求取消/超时、用量与停止原因；兼容千问返回工具块却标记 `end_turn` 的差异，同时保留显式 `max_tokens` / `stop_sequence`。可选的任务完成后“下一步建议”已经改走该适配器，但只有总开关、建议 canary 开关、精确合成白名单和用户持久化数据区域同时满足时才会调用千问；无效区域或同区域凭据缺失会返回空建议，不跨区回退，也不影响任务完成。短建议调用对已验证的 Qwen 3 max/plus/flash 系列显式关闭 thinking，对 coder 与未知模型覆盖省略不支持的参数。最终完整 Orchestrator 回归 377 个文件 / 5,911 项、typecheck、build、目标 Biome 与差异检查通过；application 安全快进部署到 `b0316b68d807112415c380b4759cdca7dd846ef8`，快速 P0 4/4、113 项运维门禁、Web lint/typecheck/build、Aliyun/Vultr 原子发布及双公网 healthz 均通过。生产 Orchestrator 单实例、uid 998、restart count 0、schema 已验证；`QWEN_MESSAGES_ADAPTER_ENABLED=false`、`QWEN_SHADOW_EVAL_ENABLED=false`、`QWEN_SUGGESTIONS_CANARY_ENABLED=false`、合成白名单数量 0，国际凭据存在、大陆凭据不存在，因此没有任何生产用户千问流量。未修改用户数据、数据库结构、支付、提现、账户关闭、DivineAPI Translator 或 OpenAI Key。

2026-09-04 JST 夜间复验补充：在目标主机用固定合成输入串联已部署的 Messages adapter 与“下一步建议”生成器，真实国际区 `qwen3.8-flash` 探针 1/1 通过；1,376ms 返回 3 条唯一且长度合规的结果，`stopReason=end_turn`，token 用量完整（302 / 20）。探针未读取用户任务、附件、数据库或日志，未记录模型正文，只在一次性进程内启用 adapter；随后生产 revision 仍为 `b0316b68`，单实例、uid 998、restart 0、schema 正常，三项 Qwen 开关仍为 false、白名单仍为 0，双公网 healthz 均为 200/status ok。

同夜真实国际端点客户端超时门禁首次发现 SDK `APIConnectionTimeoutError` 的 `constructor.name` 正确但继承的 `error.name` 为通用 `Error`，导致 adapter 3/3 误归类为 `PROVIDER_ERROR`。PR #210 以真实 SDK 错误对象先写失败测试，再用实际错误类型完成一行最小修复；目标套件 17/17、完整 Orchestrator 测试、typecheck、build、Biome 与差异检查通过。application 安全快进部署到 `180accaa7964df62e7870bb021604768376b5fbc`，schema、非 root uid 998、restart 0 与 P0 4/4 通过；部署后固定合成 1ms、`maxRetries=0` 真实国际端点探针稳定 3/3 归一为 `REQUEST_TIMEOUT`，耗时 2–76ms、平均 27ms。探针前后三项 Qwen 开关均为 false、合成白名单为 0，双公网 healthz 均为 200/status ok；未保留响应正文、请求 ID 或凭据，也未读取用户数据或修改生产配置、数据库。

千问下一调用点盘点后，产品确认不保留自动“模型二次润色层”：主模型结果应直接进入已有终态验证与前端确定性排版，避免额外时延、费用和事实被改写的风险。旧实现的 URL/数字/标记检查只是有限的单向新增与标记类型保护，不能称为完整事实保留门禁。PR #213 已移除实际润色实现，所有任务 lane 通过永久透传兼容边界返回主模型原文；旧 `OPENAI_RESPONSE_LAYER_ENABLED` 与密钥即使误配置也不能恢复第二次模型调用，新任务不再写润色审计列，历史字段仅保留兼容旧记录。完整 Orchestrator Vitest 1,349 个套件 / 5,867 项、目标回归 37/37、typecheck、build、Biome 与差异检查均通过。首次生产探针确认运行逻辑已透传，但发现 TypeScript 增量构建会遗留被删除的旧 JS；PR #214 以失败门禁锁定正常发布和回滚两条路径，统一在 build 前执行既有 clean，重新发布后旧 formatter 产物已不存在。最终 application 安全快进部署到 `052d61177394ed623b5ea4f3aa30737f90ca1098`，快速 P0 4/4、schema、单实例、uid 998、restart 0 及双公网 healthz 均通过；固定合成探针为原文不变、元数据缺失、数据库更新 0、旧产物不存在。三项 Qwen 开关仍为 false、合成白名单为 0，没有生产用户千问流量。独立的 OpenAI verifier fallback 与视频等专用调用点不属于此次范围。

千问国际区 SSE 运行时门禁已通过 PR #199、#200 收口上线：基准脚本改为增量消费响应流，分别记录首个文本增量与完整响应耗时，并新增在首个文本增量后同时触发请求 abort 与 reader cancel 的固定合成用例。审查补强后，若首 token 所在传输块已经包含 `message_delta` / `message_stop`，或标准 SSE 终止事件的 JSON 恰好跨块截断，均不会误报取消成功；超时、HTTP、网络及畸形响应的未知 token 用量保持 `null`，聚合完整性会失败关闭。最终非沙箱 Orchestrator 完整回归 374 个文件 / 5,882 项、运行时脚本 15/15、typecheck、Biome 与差异检查通过。application 从 `d5020948` 安全快进部署到 `940f4b998a0ec346f2f9b4baac7bc104f2414bd0`，快速 P0 4/4、113 项运维发布门禁、Web lint/typecheck/build、Aliyun/Vultr 原子发布及双公网 healthz 均通过；Orchestrator online、单实例、uid 998、restart count 0，数据库 schema 已验证。部署后国际区固定合成门禁 4/4：基础流首 token 586ms / 完整响应 598ms，主动取消触发 580ms / 客户端取消完成 583ms，工具两轮 1,802ms，2,048 行长上下文 2,856ms。取消后服务端 token 结算不可由客户端证明，因此该行用量明确为未知、总量标记不完整。当次 4/4 未包含真实端点客户端超时，只有注入 `AbortError` 的单元测试；该缺口后来已由上方 PR #210 与 3/3 生产合成探针补齐。QWEN_SHADOW_EVAL_ENABLED 仍为 false，未接入用户任务或切换生产流量；大陆凭据仍未配置，未触碰用户数据、AkShare、CN Payment、DivineAPI Translator 或 OpenAI Key。
...
账户关闭 rollout 的只读预检已补齐：`dormant`、`canary-ready`、`canary-running` 三种模式统一检查双公网 health、5 张 closure 表与活动队列聚合、Orchestrator/worker 数量、uid、RSS、TCP/UDP 监听端口、实际进程环境与配置文件一致性及必要配置是否存在；它会扫描 `/proc` 拦截 PM2 外孤儿 worker，兼容相对或绝对 worker 入口并单独拒绝错误工作目录。短信门槛不再只看变量是否存在，而是要求 Orchestrator 使用运行时共享密钥成功访问 CN Payment 的鉴权只读就绪端点，确认 URL、密钥、开关、凭据和两套模板形成真实可用链路。输出只允许固定检查名、布尔值、计数和长度，不打印密钥、allowlist 条目、模板 ID、用户 ID、队列内容或原始 stderr。新门禁已纳入 `test:ops`，完整运维套件 69 项 Node 测试与全部 shell 发布/回滚/运行时门禁通过。2026-08-27 JST 生产只读复验中，`dormant` 为 7/7 ready（四个开关均 false、worker 0、活动队列 0）；`canary-ready` 为 8/12 blocked，缺口严格限定为双人遗留清理确认、专用 HMAC、单一 synthetic allowlist 确认和账户关闭短信配置。该检查没有修改生产数据、进程、配置、环境变量、部署或 Git 状态，账户关闭仍未向用户开放。

账户关闭 rollout 发布链已于 2026-08-27 JST 收口：PR #153 合并只读预检与 CN Payment 鉴权健康端点；首次 CN Payment 发布在候选激活前两次遇到阿里云至官方 npm registry 的锁定依赖下载超时，未切换线上版本。PR #154 将 registry 选择限制为官方源或 npmmirror 两个 HTTPS 地址，并仅作用于远端 `pnpm install` 子进程，不写 `.npmrc`、不修改服务器全局配置且不会进入 PM2 环境。随后候选首次激活暴露此前 Pino 对连字符 header 使用非法点号 redact path 的启动缺陷，原子发布自动恢复旧 release；PR #155 改为合法 bracket notation，并用真实 pino/pino-http 覆盖启动与内部密钥脱敏。最终 CN Payment release `811d85731f91-20260827042707` 已激活：候选 6 个文件 / 41 项测试及 typecheck 通过，生产微信与支付宝门禁均 ready，进程 online，内部与公网 health 均 `status=ok`、bridge ready，临时 registry 未出现在运行环境，未认证账户关闭健康端点为 401 且 `Cache-Control: no-store`。最终账户关闭生产复验仍为 `dormant` 7/7 ready、`canary-ready` 8/12 blocked；专用 HMAC 不存在、allowlist 为空、worker 0、活动队列 0，未开启任何账户关闭或遗留清理开关，也未修改数据库、订单、用户数据、AkShare、DivineAPI、Translator 或 OpenAI Key。
...
隐私与条款的正式运营主体披露已通过 PR #140、#142 上线，并由 PR #143 收口 Vultr 双 Web 面原子发布：隐私政策与服务条款现在统一披露运营主体“上海慕雾品牌管理有限公司”、联系地址“上海市虹口区汶水东路351号B幢306室”，静态法律页更新日期为 2026 年 8 月 26 日；律师审核已由 BOSS 确认通过。生产核对确认 `holaday.ai` 与 `hd-app.orangebench.tech` 的 `/privacy`、`/terms` 均为 200，主体、地址、日期各出现 1 次，条款中的主体定义正确，旧称呼“HOLA DAY 团队（以下简称‘我们’）”为 0；两域 SPA 也均包含新的主体与地址。PR #143 修复了标准 application 发布此前只同步 Vultr SPA、却遗漏 nginx 精确法律路由实际读取的 `/opt/holaday-landing` 的发布缺口；现在 SPA 与 landing/legal 作为同一发布单元切换、内容哈希 smoke、成对回滚，并可从 HUP/INT/TERM、普通 move 失败及四个 rename 边界的 SIGKILL 中安全恢复。该修复定向 12/12、完整运维 Node 测试 50/50 与全部 shell 发布门禁通过，独立复核为 Critical 0 / Important 0。application 从 `5d6b952b3a29641e58356900b0a2161fba151c4a` 安全快进部署到 `850afb5c5f6f206960aad126ad29f4406ec301d0`；编号 migration 检查为 13 statements / 185 already applied，`db:verify`、快速 P0 发布门禁 4/4、双域 healthz 均通过，Aliyun release 为 `20260825161911-73343`，双入口 bundle 为 `index-BhM3tVIi.js`，Orchestrator 运行 uid 998、restart count 0。Vultr 远端 HEAD 与合并提交一致，Web release 状态为 `activated`，SPA 与 landing 回滚目录均存在。本轮未修改数据库 schema、生产配置、环境变量、支付供应商配置、订单或用户数据，也未触碰 AkShare、DivineAPI、Translator 或 OpenAI Key。

套餐续费与扣款口径已通过 PR #138 收口上线：实现仍是一次性付款购买固定周期、到期前由用户手动续费，不存在自动扣款；套餐页在结账前明确展示该事实，账单页统一改为“当前套餐 / 套餐购买 / 退款或提前结束套餐”，服务条款删除错误的“按月自动续费”承诺并同步更新日期，避免不同页面把同一支付行为描述成互相冲突的产品。修复同时补齐上一轮付款流水 3 个带 `aria-label` 控件缺少原生 `title` 的基线门禁。测试先行记录旧口径失败后完成修复；web-workbench 完整套件 206 个文件 / 1513 项、lint、typecheck、生产构建与 `git diff --check` 均通过。application 从 `58cf21757bf43fcffb1f3c189f639ab0b16c35b4` 安全快进部署到 `de371587d44613f7395d0f8c9bebfb7c3fc0509e`；编号 migration 检查为 13 statements / 185 already applied，`db:verify`、快速 P0 发布门禁 4/4、31 项运维测试、Aliyun/Vultr 原子 SPA smoke 与双公网 healthz 均通过，双入口 bundle 为 `index-CFOECAA_.js`，Orchestrator 运行 uid 998、restart count 0。部署后独立复核确认线上 HEAD 与合并提交一致，两个健康端点均为 `status=ok`，发布资源包含“每次付款仅购买所选周期，到期前手动续费，不会自动扣款”和“退款/提前结束套餐”，条款资源包含“4. 付费与套餐”且不存在“按月自动续费”。本轮没有改变付款捕获、套餐有效期计算、退款政策、数据库 schema、生产配置、环境变量或供应商配置；未部署 AkShare 或 CN Payment，PayPal 保持 disabled；服务条款后续已在 PR #140、#142 中按真实数据边界、正式运营主体和律师审核结果完成发布。

账单流水可信化已通过 PR #136 上线：`/billing` 现在把已支付/已退款的真实资金结果作为主区，把待确认/失败的结账尝试放入默认折叠、按需加载的“未完成支付”，两组均使用独立的租户隔离 keyset 分页和请求状态；用户可以复制公开订单号，并从具体付款记录打开预填订单、产品、金额和付款时间的凭证/发票支持邮件。服务端只返回白名单展示字段，不暴露用户标识、内部主键、网关订单/捕获号、付款人资料或 metadata。Orchestrator 支付定向 24/24、web 账单定向 28/28、双方 typecheck/build、web 全量 lint、目标 Biome 与差异检查均通过。application 从 `72ca99860190843c3e7e32061a5ec116baad2274` 安全快进部署到 `58cf21757bf43fcffb1f3c189f639ab0b16c35b4`；编号 migration 检查为 13 statements / 185 already applied，`db:verify`、快速 P0 发布门禁 4/4、31 项运维测试、Aliyun/Vultr 原子 SPA smoke 与双公网 healthz 均通过，双入口 bundle 为 `index-ckFCTezk.js`，Orchestrator 运行 uid 998、restart count 0。登录态生产 `/billing` 已确认真实成功付款只出现在“付款记录”，未完成尝试仅在展开后显示且明确“没有确认扣款”；复制反馈、凭证邮件模板、390px 与 1440px 无横向溢出、浏览器 warning/error 为 0，部署后 payment.ledger 相关错误日志计数为 0。未部署 AkShare、CN Payment 或支付供应商配置，PayPal 保持 disabled，未修改订单状态、历史数据、生产密钥、DivineAPI、Translator 或 OpenAI Key。

账号安全方案乙已通过 PR #134 完成阶段一与阶段二并上线：个人资料页现在支持向当前账号绑定邮箱发送专用验证码后自助修改密码，成功后轮换 `authVersion` 使其他设备会话失效；身份验证器 MFA 使用 RFC 6238 TOTP、5 分钟独立 challenge token、AES-256-GCM 加密 secret、只保存 keyed SHA-256 摘要的一次性恢复码、动态码重放保护和 5 次失败锁定。密码、邮箱验证码、Google OAuth、短信及密码重置登录均接入 MFA challenge，桌面与 390px 移动布局已在本地真实页面验收。Orchestrator 完整套件 287 个文件 / 4452 项、web-workbench 202 个文件 / 1499 项、CN Payment 4 个文件 / 24 项全部通过；双方 typecheck/build、web lint、MySQL MFA 集成测试及 `git diff --check` 均通过。application 从 `f6fdee06ab2f226fd1e95bea14e379846e30b434` 安全快进部署到 `72ca99860190843c3e7e32061a5ec116baad2274`；编号 migration 为 15 statements / 183 already applied，`db:verify`、快速 P0 发布门禁 4/4、31 项运维测试、Aliyun/Vultr 原子 SPA smoke 与双公网 healthz 均通过，生产 bundle 为 `index-BJmWKPwx.js`。Orchestrator 运行 uid 998；Resend 已配置；新增 MFA 主密钥仅核对存在、base64 编码长度 44、解码长度 32 及进程已加载，没有读取或输出密钥值；相关错误日志计数为 0。登录态生产 `/profile` 已只读确认“修改密码”“开启双重验证”和密码表单均可见，未发送验证码、未修改真实用户密码、未为真实用户开启 MFA，也未修改支付、AkShare、DivineAPI、Translator 或 OpenAI Key。

生产测试账号额度口径已通过 PR #132 收口上线：此前任务执行层会跳过该账号的额度扣减，但 `usage.summary`、`quota.status` 与前端仍按普通套餐展示，造成“本月 278 次执行 / 0% 已使用 / 剩余额度 100”等互相矛盾的信任问题。现已将额度模式统一为后端明确返回的 `metered | unmetered_test`；测试账号的用量页与侧栏统一显示“执行记录正常统计、额度不扣减”，普通账号与旧响应继续按计量额度处理。Web 完整套件 200 个文件 / 1492 项、Orchestrator 完整套件 286 个文件 / 4440 项、双方生产构建与 `git diff --check` 均通过。application 从 `10e0f2dad307a992176cf133f13229886634fdc7` 安全快进部署到 `f6fdee06ab2f226fd1e95bea14e379846e30b434`；编号 migration 检查为 13 statements / 183 already applied，schema 校验、快速 P0 发布门禁 4/4、Aliyun/Vultr 原子 SPA smoke 与双公网 healthz 均通过，生产 bundle 为 `index-DLiB46Tt.js`，Orchestrator 运行 uid 998。登录态生产 `/usage` 刷新后仍显示“不扣减 / 生产测试账号”及完整额度规则，侧栏同步显示“额度不扣减”，旧 `0 / 100`、`配额 100 个`、`剩余 100` 矛盾文案均未出现。该修复没有改变额度政策、普通账号扣费、用户数据、schema、生产配置、密钥或支付系统；密码变更、2FA 与交易记录信息架构保留为独立后续任务。

股市任务侧栏意图预加载已通过 PR #124 收口上线：鼠标进入或键盘聚焦“股市任务”时只预取约 89.9 kB（gzip 29.1 kB）的路由模块，不请求股票数据、不写浏览器存储；真正点击后仍只启动一组自选列表、简报状态与可信仪表盘请求，并继续复用 PR #122 的同组 promise。测试先行记录缺少路由预加载与侧栏意图回调时的失败后完成修复，定向 29/29、web-workbench 完整套件 195 个文件 / 1476 项、lint、typecheck、生产构建及 `git diff --check` 均通过。application 从 `24d0bf9cef37b71b4a142aaa290b8d823af4e3ed` 安全快进部署到 `789702bacb4aaae636ede66ab071ec0152db662f`；编号 migration 核对为 13 statements / 183 already applied、`db:verify`、快速 P0 发布门禁 4/4、31 项运维发布测试及 Aliyun/Vultr 原子 SPA smoke 均通过。Vultr 远端 HEAD 与合并提交一致，Orchestrator online、pid 1330694、uid 998、restart count 0；生产机本地及双公网 healthz 均为 `status=ok`，双站 SPA 均引用 `index-DVH-tMDO.js`。上线后的首轮记录把自动化点击 promise 固定约 3.1 秒的收尾等待误当成页面耗时；随后改为点击动作与可见内容并发计时后确认：四次独立登录态侧栏进入的页面壳为 132–189ms，中位数约 141ms，08/21 可信内容为 1,001–1,827ms，中位数约 1,022ms，而点击 promise 仍在约 3.10 秒才返回，证明 3.1 秒不是股票数据链路耗时。键盘意图预取的三次有效样本页面壳为 127–133ms、可信内容为 1,300–1,836ms，没有形成可归因的生产提速信号，因此只把 PR #124 认定为安全的小型资源预取，不宣称用户可感知收益；一次新标签直达样本页面壳约 1.94 秒、可信内容约 2.79 秒，包含整站冷启动。生产机内部交易日历接口五次缓存命中为 1.1–2.1ms，近期股票信任日志均为 current 且无刷新失败，当前没有证据支持修改后端、数据库或缓存。先前观察到的约 14.6 秒选股偏好首开本轮也未能复现（首次约 1.8 秒，缓存后约 0.74–0.86 秒）。风险证据仍稳定显示“A股检查 5/5 只”，页面控制台无 warning/error，验证后已恢复默认关注股票视图并关闭新增测试标签。验证只读浏览生产页面，没有生成股票任务、修改用户数据、schema、生产配置、密钥、支付、AkShare、DivineAPI、Translator 或 OpenAI Key；本轮性能排查到此止损，不针对测量工具的固定等待继续落码。

股市任务冷导航请求瀑布已通过 PR #122 收口上线：此前 `/stocks` 必须先下载并执行约 89.9 kB（gzip 29.1 kB）的路由模块，页面挂载后才启动自选列表、简报状态与可信仪表盘三项首屏请求；现由 lazy route loader 在请求页面模块的同时启动原有三项 tRPC 请求，页面初次挂载只消费同一组 promise，不重复请求，手动刷新与 20 秒后台刷新仍创建新请求。为守住行情时效，预取槽只允许消费一次，导航中断遗留超过 30 秒即丢弃，不写 localStorage/sessionStorage，也未预热约 14.6 秒的次要选股偏好画像。测试先行记录缺少交接模块和 30 秒过期保护时的失败后完成修复，定向 29/29、web-workbench 完整套件 195 个文件 / 1473 项、lint、typecheck、生产构建及 `git diff --check` 均通过。application 从 `5ce41700f453e7cf3b620f2f5bb58945b1902a80` 部署到 `24d0bf9cef37b71b4a142aaa290b8d823af4e3ed`；Vultr 远端 HEAD 与合并提交一致，Orchestrator PM2 pid 1322207、uid 998，本机、Vultr 与 Aliyun SPA 均为 `index-CNzHEA4o.js`，生产机本地 Orchestrator 与双公网 healthz 均为 `status=ok`。登录态生产四次独立标签页直达 `/stocks` 的旧基线为：页面壳 1,528–1,970ms、08/21 可信内容 2,531–3,427ms；新版本为页面壳 1,783–2,572ms、可信内容 2,659–3,533ms。由于网络与壳层波动，可信内容总时长中位数仅从 3,117ms 降至 2,840ms（约 8.9%）；但本次直接消除的“页面壳出现后等待可信数据”中位数从 1,303ms 降至 877ms（约 32.7%），说明串行等待已缩短但整体冷启动仍未完全收口。隐藏风险面板随后 285ms 内稳定显示“A股检查 5/5 只”，页面已恢复默认关注股票视图，控制台无 warning/error。验证只读浏览生产页面，没有生成股票任务、修改用户数据、schema、生产配置、密钥、支付、AkShare、DivineAPI、Translator 或 OpenAI Key；下一性能项应单独处理路由壳层与次要偏好画像 14.6 秒链路。

股市任务风险证据首开性能已通过 PR #120 收口上线：生产基线实测「风险证据」首次打开约 7,199ms、缓存后再次打开约 883ms，根因是风险组件及请求直到用户点击标签才开始。现改为股票工作区首屏完成后静默挂载风险面板，非当前任务使用原生 `hidden` 与 `aria-hidden` 隐藏，并在标签切换时保留组件实例；没有把约 14.6 秒的次要选股偏好画像加入预热，避免扩大首屏后台负载。测试先行验证旧行为失败后完成修复，web-workbench 定向 10/10、完整套件 194 个文件 / 1470 项、lint、typecheck、生产构建及 `git diff --check` 均通过。application 从 `0e52d6f744df87cabf9eb0ca84c0eed938b967a5` 安全快进部署到 `5ce41700f453e7cf3b620f2f5bb58945b1902a80`；编号 migration 核对为 13 statements / 183 already applied、`db:verify`、快速 P0 发布门禁 4/4、31 项运维发布测试及 Aliyun/Vultr 原子 SPA smoke 均通过，双站 bundle 为 `index-BuWt5vRm.js`，双公网 healthz 均为 200，Orchestrator online、uid 998、restart count 0。登录态生产 `/stocks` 复验显示 08/21 最新交易日数据；页面进入关注股票稳定时，隐藏风险面板已完成「A股检查 5/5 只」，点击「风险证据」到稳定内容为 320ms，较首次基线降低约 95.6%，随后已恢复默认关注股票视图。冷导航本次为 6,434ms，受网络和冷资源影响仍需独立采样，不能由本次风险预热结果推断首屏整体已优化。验证仅切换只读页面标签，没有生成股票任务、修改用户数据、schema、生产配置、密钥、支付、AkShare、DivineAPI、Translator 或 OpenAI Key。

股市任务 P2 操作尺寸审计已通过 PR #118 收口上线：登录态生产按「关注股票 → 条件选股 → 风险证据 → 交易日简报」完成桌面只读巡检，四个视图的左右键切换、焦点与 `tab` / `tabpanel` 语义均正常；审计发现共用 `SectionHeader` 操作按钮仅 18px 高，现已统一调整为触屏 44px、桌面 32px，并补齐可见焦点与 reduced-motion 适配。本地先补失败契约再修复，定向 21/21、web-workbench 完整套件 194 个文件 / 1469 项、lint、typecheck、生产构建及 `git diff --check` 均通过。application 从 `01be50ff1737ee6566f349d727412b23b30499f9` 安全快进部署到 `0e52d6f744df87cabf9eb0ca84c0eed938b967a5`；编号 migration 核对为 13 statements / 183 already applied、`db:verify`、快速 P0 发布门禁 4/4、31 项运维发布测试及 Aliyun/Vultr 原子 SPA smoke 均通过，Vultr 首次切换超时后按脚本重试成功，双站 bundle 为 `index-B0bSXoVh.js`。双公网 healthz 均为 200，Orchestrator online、uid 998、restart count 0。部署后生产简报按钮实测为 181 × 32px，主内容区没有高度低于 24px 的可见按钮，页面控制台无新增日志。验证只切换只读页面标签，没有生成股票任务、修改用户数据、schema、生产配置、密钥、支付、AkShare、DivineAPI、Translator 或 OpenAI Key；真实移动端重排与冷启动性能仍保留为后续证据项。

股市周末与盘前时间语义已通过 PR #116 收口上线：前端现在使用服务端可信的 `marketSession` 与 `dataAsOf` 统一生成 AI 状态、故事标题、简报入口、刷新标签和关注列表时间，不再在休市或盘前把上一交易日快照描述成“今日行情”。本地按测试先行记录旧文案失败后完成修复，股票定向 38/38、web-workbench 完整套件 194 个文件 / 1469 项、lint、typecheck、生产构建及 `git diff --check` 均通过。application 从 `d08689deb9151565ce52ab8b6d67a9cbbdc9d48d` 安全快进部署到 `01be50ff1737ee6566f349d727412b23b30499f9`；编号 migration 核对为 13 statements / 183 already applied、`db:verify`、快速 P0 发布门禁 4/4（12.3 秒）、31 项运维发布测试及 Aliyun/Vultr 原子 SPA smoke 均通过，Vultr 首次上传超时后按脚本重试成功。双公网 healthz 均为 200，Orchestrator online、uid 998、restart count 0。登录态生产 `/stocks` 已确认展示“休市中 · 基于 08/21 最新交易日快照”“多伦科技 08/21 交易日发生了什么”“快照刷新”“08/21 行情”和“交易日简报”；切换简报后标题为“08/21 最新交易日简报”，页面控制台无错误。验证仅切换只读页面标签，没有生成股票任务、修改行情、用户数据、schema、生产配置、密钥、支付、AkShare、DivineAPI、Translator 或 OpenAI Key。

评测任务生命周期来源隔离已通过 PR #114 收口上线：默认 `TaskRepository` 的活动任务恢复现在只读取 `origin='user'`，eval repository 只读取 `origin='eval'`；通用 smoke、暂停、恢复、确认、回复持久化和终止路径统一使用服务端签名的 `ctx.taskOrigin`，付费视频与产品历史管理仍固定为 `user`。本地按测试先行完成红绿验证，来源恢复集成 3/3、列表/详情联测 9/9、eval 生命周期 8/8、认证/上下文/仓储定向 86/86，最终 Orchestrator 完整套件 285 个文件 / 4430 项、TypeScript typecheck/build 与 `git diff --check` 均通过；大文件既有 Biome 噪音未做整文件格式重写。application 从 `41874075` 安全快进部署到 `d08689deb9151565ce52ab8b6d67a9cbbdc9d48d`，编号 migration 核对为 13 statements / 183 already applied、`db:verify` 通过，快速 P0 发布门禁 4/4（12.4 秒）、31 项运维发布测试、Aliyun/Vultr 原子 SPA smoke 均通过。最终双公网 healthz 均为 200 / `status=ok`；Orchestrator online、uid 998、restart count 0。生产受控探针只创建新的 eval-origin 测试任务：普通用户来源暂停 eval 任务返回 404 且任务仍为 executing，eval 来源暂停与终止各返回 200，终态 cancelled；未读取或修改真实用户任务。两次成功探针行均保留为 eval 验证数据且已终止，没有回填、删除或改写任何历史任务；未修改 schema、生产配置、密钥、支付、AkShare、DivineAPI、Translator 或 OpenAI Key。

P2 专业工作流性能与评测流量隔离已通过 PR #110–#112 收口上线。PR #110 将内容选题、电商日报、抖音直播复盘的完整工作流执行时间相对上一轮生产基线降低约 30%；PR #111 复用父任务已持久化的专业工作流与执行上下文，后续子任务链路耗时降低约 52%，同时保持报告、证据与 verifier 准入语义不变。PR #112 修复生产评测任务误记为 `origin='user'` 的数据边界问题：评测 runner 只通过服务端签名 JWT 传递 `taskOrigin='eval'`，tRPC context 与所有 tasks create/list/detail/follow-up/reply 路径按可信 origin 隔离，客户端不能自行提交 origin；内部非评测调用仍显式使用 `user`。生产 Orchestrator HEAD 为 `41874075b01341c37c81708fbdba2e57ea98b5a5`，运行状态 online、uid 998、restart count 0；双公网 healthz 均为 200 / `status=ok`，自动 P0 发布门禁 4/4 通过。新窗口发布门禁创建的 3 条任务均为 `origin=eval`（2 completed、1 awaiting_user），独立 P2 后续链路父子 2 条均为 completed 且 `origin=eval`，没有新增 user-origin 评测任务。P2 后续用例 1/1 通过（140.4 秒），本地 Orchestrator 完整套件 285 个文件 / 4430 项、数据库集成 9/9、TypeScript typecheck/build 与差异检查均通过。历史上已误标为 `user` 的评测行未回填、未删除；任何历史数据修复仍需单独授权与可审计方案。

P2 专业工作流生产验收已通过 PR #104–#108 收口上线：内容选题、电商日报、抖音直播复盘均使用真实任务执行、持久化验收与后续子任务链路进行验证，不以任务状态或结果自述代替证据。最终同一生产版本 `8212f47d86a4af4d7061b10a95606cdb2c8dad58` 下三套评测共 24/24 通过：`p2-content-topic` 8/8（报告 `p2-content-topic-2026-08-21T08-08-45-998Z.json`）、`p2-ecom-daily` 8/8（`p2-ecom-daily-2026-08-21T08-27-52-923Z.json`）、`p2-douyin-review` 8/8（`p2-douyin-review-2026-08-21T07-58-46-357Z.json`）。修复覆盖工作流后续指令路由、Markdown anchor 标题识别、专用电商路由不被 A 股关键词覆盖、中文“万/亿/千/百”数值归一化，以及不可改名的报告章节与可执行 Checklist 骨架；缺数据、部分数据、算术冲突与非法比例仍按预期停在 `awaiting_user`，父报告到内容日历/SOP 的后续任务均完成验收。本地 Orchestrator 定向回归 185/185、完整套件 284 个文件 / 4416 项、TypeScript typecheck/build 与差异检查均通过。application 发布第一次在阿里云 SSH 上传阶段瞬时超时，重试后 Orchestrator、Aliyun、Vultr、31 项运维门禁及 P0 快速门禁 4/4 全部通过；双公网 healthz 均为 200。生产进程状态 online、uid 998、restart count 0，`EXPERT_WORKFLOW_ENABLED`、`EXECUTION_CONTRACT_ENABLED`、`EXECUTION_VERIFIER_ENABLED`、`EVIDENCE_LEDGER_ENABLED`、`ACTION_CAPTURE_ENABLED` 均为 true；所有本轮部署与评测命令已正常退出。已知非阻断问题：真实完整报告单例约 97–204 秒，带后续子任务约 237–291 秒，正确性已达本轮上线基线，但性能需作为后续独立优化项。未修改 schema、生产配置、密钥、AkShare、CN Payment、DivineAPI、Translator 或 OpenAI Key。

基础任务验收的持久化证据门禁已通过 PR #98–#101 上线：评估器会从数据库读取聚合后的 evidence 类型、有效输出文件 MIME 与动作类型，来源、文件和真实副作用均可作为阻断断言，不再接受结果文本自述。生产快速 P0 门禁 4/4 通过（10.9 秒）；独立 `p1-persisted-gate` 2/2 通过（来源证据 46.9 秒、浏览器动作与 JPEG 产物 22.1 秒）。随后在同一生产版本运行完整 `p1-regression`，翻译、摘要、长报告、伪造网址防护、代码项目拒绝、四类真实搜索、无效域名降级及五类浏览器动作共 15/15 通过，总耗时 525.1 秒；搜索用例均要求持久化 `tool_result`，浏览用例均要求持久化 `browser_state` 与对应 navigate/click/type 动作。P0 澄清用例已改为缺少模板附件的确定性 `template_fill` 入口，避免依赖模型或专家工作流漂移。生产 Orchestrator HEAD 为 `82fdaa323d6a811980666bff99efc8403e204611`，运行 uid 998、restart count 0，`ACTION_CAPTURE_ENABLED=true`；完整 P1 后双公网 healthz 仍均为 200，部署与评测残留进程为 0。Orchestrator 完整套件 284 个文件 / 4396 项通过，最终定向回归 115/115，TypeScript typecheck/build、Biome 与差异检查通过。未修改 schema、生产密钥、AkShare、CN Payment、DivineAPI、Translator 或 OpenAI Key。
...
SPA 与 Orchestrator 已部署 `327ab0bd`（PR #58，包含前序 PR #56）。发布回滚点为 `989ddc55`，本机与生产双重祖先门禁均确认可安全快进；Orchestrator 构建、编号 migration（16 statements / 174 already applied）、`db:verify`、非 root PM2 重启、必需密钥与 healthz 检查均通过，运行用户 uid/gid 998。Aliyun 与 Vultr SPA 均命中 `index-D68mfzj9.js`，两侧原子发布 smoke、独立页面和 healthz 检查均为 200；Vultr 首次 SSH 切换超时后自动重试成功。真实登录态打开 `/cosmic` 已完整渲染今日能量、30 秒补给、星座专刊与内容流，控制台无 error。能量分析已在 2026-08-16 JST 正式开启：先在 `ENERGY_ANALYTICS_ENABLED=false` 下生成并加载独立 64 字符高熵 HMAC secret，确认登录页面仍零写入，再切为 `true` 并重启；当前进程 pid 4075681、uid 998、env 为 root-only `600`，本机与双公网 healthz 均为 200。测试账号的首页、首次开始、完成、重玩四类事件各聚合 1 次；相同重玩 UUID 重试返回 duplicate 且未重复计数，匿名访客 1 行、哈希长度 64、幂等收据 4 行。生产 `db:verify` 通过，三表共 28 列且未出现 user ID、邮箱、答案、问题、正文或 payload 列；管理员 7 天聚合响应无敏感字段，未登录查询返回 `UNAUTHORIZED`，5 个针对性测试文件共 33 项通过。微信与支付宝生产预检 ready，PayPal 按 live 配置 disabled；`SKIP_AUTO_SMOKE=1` 保持原配置，未运行会创建真实任务的 P0 auto-smoke。除新增专用能量分析 HMAC secret 外，未修改 DivineAPI、OpenAI、Translator、模型或支付配置，未部署 CN Payment 与 AKShare。

前序生产记录：SPA 与 Orchestrator 已部署 `278ae4cc`（PR #50）。今日能量在后端 8 秒预算后仍需本地回退时，会立即静默复查并最多再查两次，在真实 DivineAPI 内容可用后自动替换；页面同时提供低噪音、可访问且尊重 reduced-motion 的更新状态。发布回滚点 `d65e7f25`，两次祖先门禁均通过；Orchestrator 构建、编号 migration（13 statements / 174 already applied）、`db:verify`、非 root PM2 重启与必需密钥加载均通过，运行用户 uid/gid 998、restart count 0。Aliyun 与 Vultr SPA 均命中 `index-WNBW77fH.js`，两侧 smoke 与 healthz 200。真实用户态复验：今日内容来源为 DivineAPI，幸运色显示“琥珀金”而非十六进制；首次切换本月先显示读取状态，9 秒内自动出现真实中文内容，控制台无 warning/error。未修改 DivineAPI/OpenAI 密钥、Translator、模型或支付配置，未部署 CN Payment 与 AKShare。

更早生产记录：SPA 与 Orchestrator 曾部署 `109be03f`（PR #43 + 超时热修复 PR #44）。今日能量星座内容切换先展示真实加载态；DivineAPI 请求最长等待 8 秒，超时后自动回退本地提示，避免无限加载。生产实测从“本月”加载态在约 8–9 秒内落到“暂时使用本地提示”，刷新按钮恢复可用且页面日志为空。Orchestrator 构建、编号 migration、`db:verify`、非 root PM2 重启、必需密钥加载与 healthz 均通过；进程 `online`、restart count 0。Aliyun 与 Vultr SPA 均命中 `index-Uoin6wia.js`，两侧 healthz 200；P0 smoke 11/11 通过。Canary 浏览器配置保持原值。
