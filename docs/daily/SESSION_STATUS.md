...
## 🔴 PROD LIVE REF = `claude/musing-keller-ae1d05` — orch `572e422`（**④ browse lane wired (`--browse`) + batch-1 prep 加固 + explore-sites TDZ 修复 dark ship · restart 693 · `EXPLORER_ENABLED`+`EXPLORER_VETO_FIXTURE_ENABLED` 都 OFF（进程实证、fixture 路由 404）=绝不跑 · `--browse` LOCK1 gate 住、browser 栈仅 --browse 分支 lazy-import=用户任务不变** · cost-source A 内存 fail-closed 熔断 +（a）非有限成本 fail-closed halt +（c）`EXPLORER_MAX_ITERATIONS` env 可配(默认25/上限50) + requireBrowseEnv 缺-id abort · 护栏夹具 8/8 PASS · veto 多信号 OR · ① crystallize · migration 0037 · ACTION_CAPTURE+B4 ON · B3_FIXTURE OFF）/ SPA `8da47b4b`（bundle `index-DiYh_GAx.js`，未变）
