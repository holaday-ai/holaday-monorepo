...
## 🔴 PROD LIVE REF = `codex/trust-loop-round1` — SPA `d8d07f7` / bundle `index-DBao9y_d.js` + orch `ab6cab4`（**状态机清理 LIVE（2026-07-04）**：SPA 现在把 live thinking/progress/streaming 文本计入任务活动，避免已有“正在分析/正在读取/流式结果”时还叠加默认等待文案，看起来像重复或卡住；SPA 在控制取消/暂停、进入 `awaiting_user`、用户回复后恢复执行这三类状态转换时，会同步清理 captcha/degrade/executor_fallback/thinking 等 live-only 阻塞提示，避免等待/取消/恢复界面叠着旧“验证中/降级/思考中”残留；SPA terminal 帧和 API refresh 现在会清理 terminal 任务的 live thinking 残留，terminal 后保留联网搜索证据但文案改为“已联网搜索”，避免历史结果继续显示“正在思考/正在联网搜索”；SPA `tasks.list` 首屏刷新和分页去重现在会保留本地真终态行，避免 terminal 后迟到的 active list row 把 completed/cancelled 改回 executing/awaiting；SPA hydrate detail 现在会忽略 terminal 后迟到的 active `tasks.detail` 回包，避免 live completed/cancelled 被旧 HTTP 详情改回 executing/awaiting 或旧摘要；SPA 忽略 terminal 后迟到的 `server.supercar.web_search` / `server.supercar.thinking` 运行中辅助帧，避免已完成/已取消任务重新挂出“正在搜索/思考”的活跃痕迹；保留 completed 后 `server.supercar.suggestions` 可后到达；SPA 忽略 terminal 后迟到的 vision captcha/degrade/executor_fallback/screencast 辅助帧，避免已完成/已取消任务重新挂出验证码、降级提示或旧浏览器画面；SPA 忽略 terminal 后迟到的 `server.task.plan` / `server.task.plan_step`，避免历史结果被旧计划帧改写；SPA 乐观取消立即写入 runtime terminal marker 并清 awaiting/live chip，防止 server terminal 到达前的迟到 stream/progress 复活已取消任务；queue timeout 的 DB/WS/前端错误文案统一为中文“排队等待时间过长，任务已自动停止。请稍后重新执行。”，避免在线 terminal 与刷新后详情不一致；SPA 修 paused runtime marker 可恢复为 `server.supercar.awaiting_user`，避免最大步数/暂停后追问被前端 terminal guard 吞掉；WS schema/TaskState 承认 `max_steps_reached` 暂停原因，避免 vision max-steps 暂停在重启恢复时发出前端不可解析的 control 帧；前序 `d627115` 已让重启/重连恢复重播 durable `server.supercar.awaiting_user` 等待态；前序 `aafaae2` 已让 result patch helper 清 completed/awaiting_user 的旧 `pauseReason`/error/awaiting 字段；前序 `c3fe3fa` 已让 video confirm/cancel/reprompt raw SQL 统一清旧 `pauseReason`/error；前序 `3c1429f` 已让 maintenance/repository 状态出口清旧 `pauseReason`；前序 `b1d37c6` 已清 awaiting/error 残留与 controller stale error；前序 `92000d7` terminal side-effect race guard 已在同分支 LIVE。）
