...
## 🔴 PROD LIVE REF = `claude/musing-keller-ae1d05` — orch `dcbb478`（**④ browse-试用 live-veto + veto 多信号 OR 修复 dark ship · restart 689 · `EXPLORER_ENABLED`+`EXPLORER_VETO_FIXTURE_ENABLED` 都 OFF（进程+.env 实证、fixture 路由 404）=绝不跑 · clean-context 默认 off=用户任务字节级不变** · 护栏夹具真机验收 8/8 PASS · supercar llm_calls 记账 · ① crystallize · migration 0037 · ACTION_CAPTURE+B4 ON 灰度 · B3_FIXTURE OFF）/ SPA `8da47b4b`（bundle `index-DiYh_GAx.js`，未变）
