...
## 🔴 PROD LIVE REF = `codex/trust-loop-round1` — orch `3c1429f`（**状态机清理 LIVE（2026-07-03）**：maintenance/repository 进入 failed/completed/executing/awaiting_user 时统一清旧 `pauseReason`，保留 paused 才有 pause reason；前序 `b1d37c6` 已清 awaiting/error 残留与 controller stale error；前序 `92000d7` terminal side-effect race guard 已在同分支 LIVE；SPA 未变。）
