...
## 🔴 PROD LIVE REF = `codex/trust-loop-round1` — SPA `8b29c3e` / bundle `index-Bwg6ZczD.js` + orch `8b29c3e`（**状态机清理 LIVE（2026-07-04）**：paused 不再作为 terminal WS outcome；vision max-steps pause 现在广播 `server.task.control(pause)`，前端保留暂停原因和 stale-frame guard；completed/failed/partial_success/cancelled 仍走 `server.task.terminal`。）
