...
## 🔴 PROD LIVE REF = `codex/trust-loop-round1` — SPA `2dd384de` / bundle `index-DBBSuxe1.js` + orch `ff0634f8`（**状态机防复活/防误恢复 guard + 技能 planner 闭环 LIVE（2026-07-09）**：controller 层禁止 `start(existing)` 重启 planning/pending/queued/awaiting/paused/terminal，late step result 只在 executing 接受；新任务 seed 显式走 `state:null + taskId + plan`；pause 只允许 executing source；repository 层 `applyStepResult` 只允许 executing/awaiting_user source，control transition 拒绝 terminal source、非 executing→paused 与非 paused→executing，batch approve 只允许等待批量确认的任务恢复到 executing；`tasks.reply` 对仍等待中的视频报价任务重播保留 `video_quote` 等待类型。技能页 shared 13 技能与 planner catalogue 已对齐：DB-backed SKILL.md 保留 allowedOrigins，缺 DB rows 的用户可见技能以描述型 fallback 进入 planner；手动 @ 技能会注入 planner hint。此前未成功任务清理口径、批量任务需复核计数拆分、技能 dispatch、技能目录与 @ 技能入口、批量任务未成功口径、用量加载态、任务详情 evidence fallback、批量子任务状态保真、股票最近真实交易日分时、状态机清理与通知遮挡修复继续生效。）
