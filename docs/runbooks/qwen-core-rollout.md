...
仅当生产还没有任何可用的 Qwen-only 回滚提交时，允许执行一次显式的暗发布引导：

1. 先把生产运行环境设置为 `MODEL_RUNTIME_POLICY=qwen_only`、`QWEN_CORE_ROLLOUT_MODE=off`，并配置国际专用凭据和两个固定端点；不得配置任何用户白名单。
2. 候选提交必须含生产 `qwen_only` 强制校验，且国际协议与运行时探针已在本地通过。
3. 使用 `ALLOW_INITIAL_QWEN_CUTOVER=1` 执行标准 Orchestrator 部署。此参数仅放宽“旧回滚提交缺少 Qwen-only 标记”这一项，不放宽祖先关系、构建、健康、进程身份、运行环境或暗发布门禁。
4. 部署脚本必须在 `rollout=off` 下执行生产导入图、国际协议和国际运行时暗门禁；任何失败都立即停止。
5. 首次切换失败时，只允许恢复部署前提交并在恢复进程中强制清空 Anthropic、OpenAI、Gemini 和 Google 凭据。该路径用于恢复服务，不得承载模型用户流量；恢复失败则进入人工处置，不得自动启用旧供应商。

首次暗发布成功后，该提交即成为 Qwen-only 回滚基线；后续部署不得再使用首次切换参数，也不得回滚到更早的旧模型提交。
...
除上述一次性、`rollout=off` 的首次落点外，若不存在合格的 Qwen-only 回滚提交，停止发布并保留现状，不能回退到会重新调用旧模型的版本。
