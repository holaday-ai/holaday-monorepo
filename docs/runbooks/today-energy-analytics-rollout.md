...
- 清理任务不受写入开关控制。关闭新写入后，过期数据仍会每小时按既定单次上限清理；若删满上限，会自动接续有界批次直到追平积压。
...
9. 如需回滚，把 `ENERGY_ANALYTICS_ENABLED=false` 并重启或发布 Orchestrator。确认新事件不再增加聚合数；保留三张表和每小时清理，不删除表、不清空历史数据。
...
- 清理连续删满单次上限后没有在 1 秒后接续，或积压行数持续增长；
