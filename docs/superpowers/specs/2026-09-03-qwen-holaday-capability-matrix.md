...
|---|---|---|---|---|
| 1 | `response-layer/openai-response-layer.ts` | 将长任务结果润色后展示；现有开关默认关闭，异常回退原文，并有 URL、数字和标记保留校验 | **中**：无工具、无外部 mutation，但会接触并改写用户结果；错误润色直接影响可信度 | 独立 Qwen flag、用户数据区域硬门、精确合成白名单、固定合成原文/结果对照、中文风格与事实保留门禁、原文可回退；不得把 shadow 正文写入日志或数据库 |
| 2 | `agent/supercar/plan-service.ts` | 在执行前生成用户可见步骤与预计耗时 | **中高**：计划会塑造用户预期；目前安全红线主要在 prompt，缺少对禁止动作与步骤结构的确定性后验闸 | 先建立计划 schema、危险动词拒绝、步骤数/工具标签/耗时校验，再做合成 canary；任何失败回退为无计划，不影响主任务 |
...
下一步建议只进入“响应润色层”规格确认：它是现存调用点中唯一同时具备默认关闭、无工具、失败回退原文和确定性事实后验检查的候选。这里没有授权代码接入、生产切流或用户内容 shadow；产品侧先确认是否继续保留“模型二次润色”这一层，再决定做迁移还是直接下线该层。
