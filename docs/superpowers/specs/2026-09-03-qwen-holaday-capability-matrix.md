...
| 2（已实现，未启用） | `agent/supercar/plan-service.ts` | 在执行前生成用户可见步骤与预计耗时 | **受控中风险**：计划只影响用户预期，不驱动工具；模型原文不直接展示，必须通过结构与危险动作后验闸 | 已建立 2-6 步 JSON schema、工具标签/耗时范围、危险最终动作拒绝、确定性 UI 渲染、独立 plan canary、精确合成白名单和持久化区域硬门；失败只回退为无计划，不跨区或跨 provider 回退 |
...
产品决策：不迁移自动响应润色层。主模型直接产出结果，结构与可读性由主提示词、确定性前端模板和已有终态验证负责；若未来某个场景确需改稿，应作为用户主动触发、范围明确的独立能力重新设计，不能恢复成所有任务的自动二次调用。`plan-service.ts` 已获授权进入独立合成 canary 实现，但默认关闭且空白名单，不构成生产用户切流；再下一候选 `url-resolver.ts` 仍只有风险盘点，没有接入授权。
...
- 已实现待发布：任务计划改走 provider-neutral adapter；Qwen 需要总开关、独立 plan canary、精确合成白名单和持久化数据区域同时满足，模型原文还必须通过结构化 schema 与危险动作后验闸。
