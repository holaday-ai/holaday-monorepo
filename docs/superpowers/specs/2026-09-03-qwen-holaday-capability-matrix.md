...
| 已安全退出（待发布） | `agent/url-resolver.ts` | 用户明确 URL 经网络安全校验后保留；唯一可信站点映射直接注入；未知或歧义名称要求先搜索核验 | **已消除模型裁决风险**：Claude 与千问均不再猜测导航目标；协议、嵌入凭据、DNS 与内网边界失败关闭 | 发布前验证用户 URL、可信映射、未知/歧义名称、危险 URI、私网解析与隐私日志；生产探针只使用固定合成输入，不执行浏览器导航 |
...
产品决策：不迁移自动响应润色层。主模型直接产出结果，结构与可读性由主提示词、确定性前端模板和已有终态验证负责；若未来某个场景确需改稿，应作为用户主动触发、范围明确的独立能力重新设计，不能恢复成所有任务的自动二次调用。`plan-service.ts` 的独立合成 canary 已实现、发布并通过国际区固定合成探针，但生产开关默认关闭且白名单为空，不构成生产用户切流。`url-resolver.ts` 同样不迁移到千问：目标站点属于安全与事实边界，已改为用户 URL + 可信站点映射 + 未知名称先搜索核验的确定性路径，当前等待发布门禁。
...
- 已实现待发布：URL 解析不再由任何模型猜测；用户 URL 与可信映射统一通过协议、嵌入凭据、DNS 和私网安全闸，未知或歧义名称只生成搜索核验指令，运行日志只保留结果分类与拒绝原因。
