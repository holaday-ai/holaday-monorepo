...
产品决策：不迁移自动响应润色层。主模型直接产出结果，结构与可读性由主提示词、确定性前端模板和已有终态验证负责；若未来某个场景确需改稿，应作为用户主动触发、范围明确的独立能力重新设计，不能恢复成所有任务的自动二次调用。下一候选变为 `plan-service.ts`，但本表仍只完成风险盘点，没有授权代码接入、生产切流或用户内容 shadow。
