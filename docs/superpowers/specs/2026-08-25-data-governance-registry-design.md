...
首阶段使用 TypeScript 常量和只读类型，不使用数据库动态配置。这样可以让编译、单元测试和代码审查共同约束变更，并避免运行时配置与真实代码漂移。源码符号证据使用就近项目 `tsconfig`、TypeScript `Program` 和 `TypeChecker` 做语法、语义及导出值核验；审计器绝不 import 或执行被核验模块。
...
8. `unknown` 必须有原因；
9. 源码证据必须指向仓库内存在的路径；若指定导出符号，审计器必须确认该 TypeScript 源文件及相关程序没有错误级语法或语义诊断，并从模块符号的导出表中找到同名、本地、可发射的 value symbol；
...
注册表在模块加载时只进行类型约束，不产生网络或数据库副作用。审计器聚合全部错误后一次性输出，避免修复一个问题后才发现下一个。源码符号核验只构建 TypeScript 编译程序并读取诊断及符号表，不动态 import、执行或求值被审计模块。

源码符号合同证明的是“所引用文件在对应 TypeScript 编译上下文中通过语法与语义检查，且精确导出名解析为本地可发射的值声明”。它不承诺模块在任意运行环境加载时绝不会因顶层业务逻辑、外部服务或其他运行时条件抛错。默认导出、`export =`、外部重导出、导入别名、ambient 声明、可擦除的 `const enum`、声明文件和非 TypeScript 源格式在首阶段均不作为可证明证据，必须失败关闭。
...
- 未知状态必须有原因；
- 已实现能力缺少处理器失败。
- 导出证据所在程序存在重复绑定、重复导出、缺失引用、缺失基类或其他错误级 TypeScript 诊断时失败；
- 注释、类型导出、默认导出、外部别名和不支持的文件格式不得冒充精确的本地值导出。
