...
- `astrology`: disclose the account-scoped localStorage profile (name, exact birthday, optional birth time/place, zodiac), browser timezone sent to protected HOLA DAY astrology endpoints, the smaller sign/date-or-period/language/timezone field set sent to DivineAPI when live capability is enabled, and the existing “我的能量 > 清除资料” control.
- `external favicon`: do not automatically contact Google's favicon service from search results or admin domain pages; use local globe icons so this browser-side third-party transfer does not occur.
